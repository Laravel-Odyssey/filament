...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i '.basename(__FILE__).' | findstr /r /i /M /F:/ ":author :vendor :package LaravelOdyssey Filament migration_table_name vendor_name vendor_slug author@domain.com"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i ":author|:vendor|:package|LaravelOdyssey|Filament|migration_table_name|vendor_name|vendor_slug|author@domain.com" --exclude-dir=vendor ./* ./.github/* | grep -v '.basename(__FILE__)));
}
...
$LaravelOdyssey = ask('Vendor name', $guessGitHubVendorInfo[0]);
$vendorUsername = ask('Vendor username', $guessGitHubVendorInfo[1] ?? slugify($LaravelOdyssey));
$vendorSlug = slugify($vendorUsername);

$LaravelOdysseyspace = str_replace('-', '', ucwords($LaravelOdyssey));
$LaravelOdysseyspace = ask('Vendor namespace', $LaravelOdysseyspace);
...
writeln("Vendor     : {$LaravelOdyssey} ({$vendorSlug})");
writeln("Package    : {$packageSlug} <{$description}>");
writeln("Namespace  : {$LaravelOdysseyspace}\\{$className}");
...
foreach ($files as $file) {
    replace_in_file($file, [
        ...
        ':vendor_name' => $LaravelOdyssey,
        'Laravel-Odyssey' => $vendorSlug,
        'LaravelOdyssey' => $LaravelOdysseyspace,
        ':package_name' => $packageName,
        'filament' => $packageSlug,
        'filament_without_prefix' => $packageSlugWithoutPrefix,
        'Filament' => $className,
        'Filament' => $packageSlug,
    ...
    match (true) {
        str_contains($file, determineSeparator('src/Filament.php')) => rename($file, determineSeparator('./src/'.$className.'.php')),
        str_contains($file, determineSeparator('src/FilamentServiceProvider.php')) => rename($file, determineSeparator('./src/'.$className.'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/Facades/Filament.php')) => rename($file, determineSeparator('./src/Facades/'.$className.'.php')),
        str_contains($file, determineSeparator('src/Commands/FilamentCommand.php')) => rename($file, determineSeparator('./src/Commands/'.$className.'Command.php')),
        str_contains($file, determineSeparator('database/migrations/create_Filament_table.php.stub')) => rename($file, determineSeparator('./database/migrations/create_'.title_snake($packageSlugWithoutPrefix).'_table.php.stub')),
        str_contains($file, determineSeparator('config/Filament.php')) => rename($file, determineSeparator('./config/'.$packageSlugWithoutPrefix.'.php')),
